...
def calculate_rouge(generated, references):
    ...
    for x in(rouge_scores):
        summ+=(x-avgg)*(x-avgg)
    return sum(rouge_scores) / len(rouge_scores), math.sqrt(summ/len(rouge_scores))
...
def calculate_meteor(generated, references):
    ...
    for x in(meteor_scores):
        summ+=(x-avgg)*(x-avgg)
    return sum(meteor_scores) / len(meteor_scores), math.sqrt(summ/len(meteor_scores))


input_file = "./summary_results_1.jsonl"
output_file = "./summary_results_1p.jsonl"
