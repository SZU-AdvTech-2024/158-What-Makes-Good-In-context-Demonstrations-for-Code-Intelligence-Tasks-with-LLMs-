...
def sbert_preprocess(train, test, model_path, output_path, number):
    ...
    print("22\n")
    code_emb = model.encode(code, convert_to_tensor=True, show_progress_bar=True, device='cuda' if torch.cuda.is_available() else 'cpu')
    print("44\n")
    processed = []
...
train = []
with jsonlines.open('C:/Users/Dell/Desktop/qian/ICL4code-master/data/szgao/resource/dataset/CodeSearchNet/java/train.jsonl') as f:
    for i in f:
        train.append(i)
test = []
with jsonlines.open('C:/Users/Dell/Desktop/qian/ICL4code-master/data/szgao/resource/dataset/CodeSearchNet/java/test.jsonl') as f:
    for i in f:
        test.append(i)
